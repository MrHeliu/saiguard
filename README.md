...
> **v2.7 Update**: PRD requirements traceability — `sai start` now auto-links PRD documents to task plans, ensuring AI implementation matches actual requirements.
...
- **Two-stage verification** — build pass + test pass (configurable `testCmd` per agent type)
- **PRD traceability** — `sai start` auto-scans PRD directory, generates plan with required requirements coverage section
...
- **i18n** — locale-driven output (`en`/`zh`), configurable in `config.json`
...
> **v2.7 更新**：PRD 需求追溯 — `sai start` 自动关联 PRD 文档到任务计划，确保 AI 实现符合实际需求。
...
- **两阶段验证** — 构建通过 + 测试通过（每个 agent 类型可配置 `testCmd`）
- **PRD 需求追溯** — `sai start` 自动扫描 PRD 目录，生成带需求覆盖区块的实施计划
...
三个输入驱动系统：
- **PRD** — 做什么（需求来源，`sai start` 时自动关联）
- **Rules** — 怎么做（15 条硬约束，每次会话加载）
